...
export async function deleteUser(request, response) { if (String(request.user._id) === request.params.id) throw new AppError(400, 'You cannot delete your own account'); const owned = await Prompt.find({ creator: request.params.id }).distinct('_id'); await Promise.all([User.findByIdAndDelete(request.params.id), Session.deleteMany({ user: request.params.id }), Prompt.deleteMany({ creator: request.params.id }), Bookmark.deleteMany({ $or: [{ user: request.params.id }, { prompt: { $in: owned } }] }), Review.deleteMany({ $or: [{ user: request.params.id }, { prompt: { $in: owned } }] }), Report.deleteMany({ $or: [{ reporter: request.params.id }, { prompt: { $in: owned } }] })]); response.status(204).end(); }
