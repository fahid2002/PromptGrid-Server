...
export async function createCheckout(request, response) {
  const returnPath = typeof request.body.returnPath === 'string' && request.body.returnPath.startsWith('/') ? request.body.returnPath : '/dashboard/profile';
  const session = await stripe.checkout.sessions.create({
    mode: 'payment',
    customer_email: request.user.email,
    line_items: [{ quantity: 1, price_data: { currency: 'usd', unit_amount: 500, product_data: { name: 'PromptGrid Premium Access', description: 'One-time unlock for every private prompt' } } }],
...
export async function webhook(request, response) {
  let event;
  try { event = stripe.webhooks.constructEvent(request.body, request.headers['stripe-signature'], env.STRIPE_WEBHOOK_SECRET); } catch { throw new AppError(400, 'Invalid Stripe webhook signature'); }
...
export async function sessionStatus(request, response) {
  ...
  if (session.metadata?.userId !== String(request.user._id)) throw new AppError(403, 'This payment session belongs to another user');
  if (session.payment_status === 'paid') await fulfillCheckoutSession(session);
  response.json({ paymentStatus: session.payment_status, premium: session.payment_status === 'paid', payment: await Payment.findOne({ stripeSessionId: session.id }) });
}
