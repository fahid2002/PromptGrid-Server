...
describe('authenticated marketplace flow', () => {
  it('registers, logs in by cookie, enforces the free limit, and protects premium content', async () => {
    ...
    const login = await agent.post('/api/auth/login').send({ email: 'course@example.com', password: 'securepass123' }).expect(200);
    expect(login.headers['set-cookie'][0]).toContain('promptgrid_token=');
  ...
  it('lets an admin change roles, moderate prompts, inspect pagination, and warn a creator', async () => {
    ...
    await admin.post('/api/auth/login').send({ email: 'admin@example.com', password: 'adminpass123' }).expect(200);
